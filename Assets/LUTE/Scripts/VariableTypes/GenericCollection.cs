...
/// <summary>
/// The meat of the Collection. Internally uses a List of given type, simplest example
...
/// IntCollection an int or a IntVariable, either works as the Promote is aware
/// of BaseVariable<T>. Will also allow mixing some operations between
...
public class GenericCollection<T> : Collection
{
    ...
    {
        if (variable is BaseVariable<T>)
        {
            BaseVariable<T> vt = variable as BaseVariable<T>;
    ...
    public override bool IsCollectionCompatible(object o)
    {
        ...
        {
            return ote is T || ote is BaseVariable<T>;
        }
        else if (o is System.Collections.IList && otgs.Length > 0)
        {
            return otgs[0] == typeof(T) || otgs[0].IsSubclassOf(typeof(BaseVariable<T>));
        }
        else
        {
            return false;
    ...
    {
        return o is T || o is BaseVariable<T>;
    }
    ...
    protected virtual T Promote(object o)
    {
        ...
        else if (o is BaseVariable<T>)
        {
            var oAs = o as BaseVariable<T>;
            return (T)oAs.Value;
        }
