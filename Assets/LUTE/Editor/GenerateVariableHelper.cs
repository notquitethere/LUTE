...
{
    /// <summary>
    /// Helper script that auto generates the required script for LUTEVariables that wrap existing classes.
    ///
    /// Intended to speed up the creation of LUTE wrappers of unity builtin types and custom user types.
    /// </summary>
    public class VariableScriptGenerator
    {
        ...
        //data and helper for a single native to single type
        public class LUTEVariableTypeHelper
        {
            ...
            {
                public TypeHandler(Type native, Type LUTEtype, string localName, string nativeAsString = null, string LUTETypeAsString = null)
                {
                    NativeType = native;
                    NativeTypeString = string.IsNullOrEmpty(nativeAsString) ? native.Name : nativeAsString;
                    LUTEType = LUTEtype;
                    LUTETypeString = string.IsNullOrEmpty(LUTETypeAsString) ? LUTEtype.Name : LUTETypeAsString;
                    LocalVariableName = localName;
                }

                public Type NativeType { get; set; }
                public Type LUTEType { get; set; }
                public string NativeTypeString { get; set; }
                public string LUTETypeString { get; set; }
                public string LocalVariableName { get; set; }

                public string GetLocalVariableNameWithDeclaration()
                {
                    return "var " + LocalVariableName + " = inOutVar as " + LUTETypeString + ';';
                }

                public string GetVarPropText()
                {
                    return "typeof(" + LUTETypeString + ")";
                }
            }
        ...
        private LUTEVariableTypeHelper helper = new LUTEVariableTypeHelper();
        ...
        private const string ScriptLocation = "./Assets/LUTE/Scripts/";
        private const string PropertyScriptLocation = ScriptLocation + "Orders/Property/";
        private const string VaraibleScriptLocation = ScriptLocation + "VariableTypes/";
        private const string EditorScriptLocation = "./Assets/LUTE/Editor/VariableTypes/";
...
{5}
        ...
        public VariableScriptGenerator()
        {
            ...
            helper.AddHandler(new LUTEVariableTypeHelper.TypeHandler(typeof(Collection), typeof(CollectionVariable), "iocollect"));
            ...
            helper.AddHandler(new LUTEVariableTypeHelper.TypeHandler(typeof(float), typeof(FloatVariable), "iof"));
            //helper.AddHandler(new FungusVariableTypeHelper.TypeHandler(typeof(GameObject), typeof(GameObjectVariable), "iogo"));
            helper.AddHandler(new LUTEVariableTypeHelper.TypeHandler(typeof(int), typeof(IntegerVariable), "ioi"));
